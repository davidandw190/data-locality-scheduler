...
class BenchmarkVisualizer:
    ...
    def visualize_network_topology_impact(self, filename='network_topology_impact.png'):
        ...
        ax1.set_xlabel('Transfer Distance', fontsize=13, fontweight='bold')
        ax1.set_ylabel('Total Data Volume (MB)', fontsize=13, fontweight='bold')
        ax1.set_title('Data Transfer Volume by Network Distance', fontsize=15, fontweight='bold')
        ax1.set_xticks(x)
        ax1.set_xticklabels(transfer_types, fontsize=12)
        ax1.legend(loc='upper right', fontsize=12, frameon=True, fancybox=True, shadow=True)
        ax1.grid(True, alpha=0.3)
        ax1.tick_params(axis='both', which='major', labelsize=11)
        
        # Plot 2: Network Cost Model
        # Cost multipliers: Local=1, Same Region=5, Cross Region=10
        ...
        for i, (t, color) in enumerate(zip(transfer_types, colors_cost)):
            dl_bar = ax2.bar(positions[0], dl_cost_breakdown[i], bar_width, bottom=bottom_dl, 
                            color=color, edgecolor='black', linewidth=1.2, 
                            label=t if i == 0 else "", alpha=0.9)
            
            default_bar = ax2.bar(positions[1], default_cost_breakdown[i], bar_width, bottom=bottom_default, 
                                color=color, edgecolor='black', linewidth=1.2, alpha=0.7)
            
            if dl_cost_breakdown[i] > 100:  
                label_color = 'white' if i == 2 else 'black'  
                ax2.text(positions[0], bottom_dl + dl_cost_breakdown[i]/2, 
                        f'{dl_cost_breakdown[i]:.0f}',
                        ha='center', va='center', fontsize=11, fontweight='bold',
                        color=label_color)
            
            if default_cost_breakdown[i] > 100:
                label_color = 'white' if i == 2 else 'black'
                ax2.text(positions[1], bottom_default + default_cost_breakdown[i]/2, 
                        f'{default_cost_breakdown[i]:.0f}',
                        ha='center', va='center', fontsize=11, fontweight='bold',
                        color=label_color)
        ...
        max_height = max(bottom_dl, bottom_default)
        total_label_offset = max_height * 0.08  
        reduction_label_offset = max_height * 0.2  
        
        ax2.text(positions[0], bottom_dl + total_label_offset, 
                f'Total: {dl_cost:.0f}', ha='center', va='bottom', 
                fontsize=12, fontweight='bold', 
                bbox=dict(boxstyle="round,pad=0.3", facecolor='white', alpha=0.8))
        
        ax2.text(positions[1], bottom_default + total_label_offset, 
                f'Total: {default_cost:.0f}', ha='center', va='bottom', 
                fontsize=12, fontweight='bold',
                bbox=dict(boxstyle="round,pad=0.3", facecolor='white', alpha=0.8))
        
        reduction = (default_cost - dl_cost) / default_cost * 100 if default_cost > 0 else 0
        ax2.text(np.mean(positions), max_height + reduction_label_offset, 
        ...
        ax2.set_ylabel('Network Cost (Arbitrary Units)', fontsize=13, fontweight='bold')
        ax2.set_title('Estimated Network Cost Comparison', fontsize=15, fontweight='bold')
        ax2.set_xticks(positions)
        ax2.set_xticklabels(['Data Locality\nScheduler', 'Default\nScheduler'], fontsize=12)
        ax2.set_xlim(-0.4, 1.6)
        
        ax2.set_ylim(0, max_height + reduction_label_offset + max_height * 0.1)
        
        legend_elements = [plt.Rectangle((0,0),1,1, facecolor=c, edgecolor='black', alpha=0.9) 
                        for c in colors_cost]
        legend = ax2.legend(legend_elements, transfer_types, 
